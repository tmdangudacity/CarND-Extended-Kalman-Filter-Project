...
int main(int argc, char* argv[])
{
    ...
    // Create an instance of FusionEKF

    // Default option is using Laser and Radar data
    FusionEKF fusionEKF;

    if(data_option == "-L")
    {
        fusionEKF = FusionEKF(FusionEKF::LASER_DATA_ONLY);
    }
    else if(data_option == "-R")
    {
        fusionEKF = FusionEKF(FusionEKF::RADAR_DATA_ONLY);
    }
        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2')
        {
            ...
            if (s != "")
            {
                ...
                if (event == "telemetry")
                {
                    ...
                    std::cout << "Estimation size: " << estimations.size()
                              << ", RMSE: " << RMSE(0)
                              << ", "       << RMSE(1)
                              << ", "       << RMSE(2)
                              << ", "       << RMSE(3)
